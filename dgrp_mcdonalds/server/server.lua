...
RegisterServerEvent('dgrp_mcdonalds:giveOrder')
	AddEventHandler('dgrp_mcdonalds:giveOrder', function(receiverID, item1, item2, item3)
	local source = ESX.GetPlayerFromId(source)
	local reveiver
	if receiver == nil then
		print("receiver is nil - Let's Set it")
		receiver = ESX.GetPlayerFromId(receiverID)
		if receiver ~= nil then
			print("receiver sucessfully set to "..receiver.." it should be "..receiverID)
		else
			print("receiver is still nil! lets try set it again! it should be set to "..receiverID)
			receiver = ESX.GetPlayerFromId(receiverID)
		end
	else
		receiver = 0
		receiver = ESX.GetPlayerFromId(receiverID)
		print("receiver is not equal to nil - not setting")
	end

	if source ~= nil then
		source.removeInventoryItem(item1, 1)
		source.removeInventoryItem(item2, 1)
		source.removeInventoryItem(item3, 1)
	else
		print("source is Nil unable to transfer Items.")
	end
	...
	if receiver ~= nil then
		...
		print("Receiver is Nil unable to transfer Items. Giving Back Items")
		source.addInventoryItem(item1, 1)
		source.addInventoryItem(item2, 1)
		source.addInventoryItem(item3, 1)
	end
end)

--------------------------------------END OF ADVANCED BETA TESTING--------------------------------------


--Simple Mode Items
if Config.EnableAdvancedMode == false then
	...
	--Ashton324's Advanced Mode Items
	...
	ESX.RegisterUsableItem('MCDS_HASHBROWN', function(source)
		local xPlayer = ESX.GetPlayerFromId(source)
		xPlayer.removeInventoryItem('MCDS_HASHBROWN', 1)

		TriggerClientEvent('esx_status:add', source, 'hunger', 200000)
		TriggerClientEvent('esx_basicneeds:onEat', source)
		xPlayer.showNotification(_U('used_mcdshasbrown'))
	end)
